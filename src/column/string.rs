...
impl StringColumnData {
    ...
    fn range(&self, index: usize) -> Result<(usize, usize)> {
        let start = if index == 0 {
            ...
            self.offsets.get(index - 1).copied().ok_or_else(|| {
                crate::error::Error::Protocol("StringColumnData: index out of bounds".into())
            })? as usize
        };
    ...
    pub fn get_bytes(&self, index: usize) -> Result<&[u8]> {
        ...
        self.data
            .get(start..end)
            .ok_or_else(|| crate::error::Error::Protocol("StringColumnData: invalid range".into()))
    }
    ...
    pub fn get_str(&self, index: usize) -> Result<&str> {
        ...
        std::str::from_utf8(bytes).map_err(|e| {
            crate::error::Error::Protocol(format!("invalid UTF-8 at row {index}: {e}"))
        })
    }
