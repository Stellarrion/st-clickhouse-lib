...
pub struct PointColumnData<'a> {
    data: crate::column::plain::PlainColumnData<'a, f64>,
    count: usize,
}
...
impl ClickHouseColumn for Point {
    ...
    fn read_column<'a>(ctx: &mut ReadColumnContext<'a>) -> Result<Self::ColumnData<'a>> {
        ...
        let nbytes = count
            .checked_mul(16)
            .ok_or_else(|| crate::error::Error::Protocol("Point column size overflow".into()))?;
        let bytes = ctx.read_exact(nbytes)?;
        let inner_data =
            crate::column::plain::PlainColumnData::<f64>::read_from_bytes(bytes, count * 2);
        Ok(PointColumnData {
            data: inner_data,
            count,
        })
        ...
        if rows == 0 {
            return Ok(RingColumnData {
                ...
                points: PointColumnData {
                    data: crate::column::plain::PlainColumnData::empty(),
                    count: 0,
                },
            });
        }
...
mod tests {
    ...
    fn test_ring_column() {
        ...
        // Points in Tuple column-major form: x values, then y values.
        buf.extend_from_slice(&1.0f64.to_le_bytes());
        buf.extend_from_slice(&3.0f64.to_le_bytes());
