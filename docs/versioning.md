...
This document is the authoritative versioning manifest for `st-clickhouse-lib`
and its companion crates. CI enforces it; the enforcement lives in
`.github/workflows/ci.yml` (jobs `version-check` and `version-bump-check`).
...
compare against the crates.io API, semver-aware). Consequences:
...
- Never reuse or lower an already-published version (also refused by the
  registries themselves; the PR gate fails fast with a better message).
...
- **Minor** (`0.4.0`): new features, new APIs, deprecations. May carry
  ...
  in `CHANGELOG.md`.
- **Major** (`1.0.0`): reserved for the stability commitment (API freeze
  guarantees, MSRV policy).
